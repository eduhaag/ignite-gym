...
describe('Validate check-in use case', () => {
  ...
  it('should not be able to validade an inexisting check-in', async () => {
    expect(async () => {
      await sut.execute({ checkInId: 'non-existing-check-in-id' })
    }).rejects.toBeInstanceOf(ResourceNotFoundError)
  })

  it('should not be able to validate the check-in after 20 minutes of its creation', async () => {
    vi.setSystemTime(new Date(2023, 2, 13, 13, 40, 0))
